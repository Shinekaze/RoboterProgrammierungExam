...
for i in range(1,9) :
    ...
    entrance = LineString([(11.5, 11.5), (11.5 + np.sin(np.deg2rad(360.0 / i) * (-1)**i) * (1.0 * i), 11.5 + np.cos(np.deg2rad(360.0 / i) * (-1)**i) * (1.0 * i))]).buffer(0.2)
...
# -----------------------------------------
scene = dict()
scene['K'] = Polygon([(0, 8), (0, 17), (3, 17), (3, 13.5), (5.5, 17), (9, 17), (5.5, 12.5), (9, 8), (5.5, 8), (3, 11.5), (3, 8)])
...
goal = [11.5, 2]
difficulty = 2
benchList.append(Benchmark("Zigzag_medium", CollisionChecker(benchmark_medium), [start], [goal], description, difficulty))
